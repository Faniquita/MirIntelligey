...
char diasSemana[8][10] = {"Domingo", "Segunda", "Terca", "Quarta", "Quinta", "Sexta", "Sabado", "Error"};
int cnt = 0;
...
void update_wheater_Data(){
  ...
  //Imagens do dia
  //str_Weather_Icon = doc_API_Wheater["current"]["condition"]["icon"].as<String>();
  //str_Weather_Icon.replace("\"", "");

  //Imagens dos dias da semana
...
void setup() {
  ...
  Serial.println(doc_API_Wheater["forecast"]["forecastday"].size());  // verificar tamanho do array

  //Iniciando Relogio: Data e hora de acordo com o servidor 
  update_DateTime();  

  // Atualizar imagens do clima
  Serial.println("Iniciando renderização das imagens...");
  update_wheater_Data();
  Serial.println("Finalizando renderização das imagens");

  // Atualizando dados do Tempo
  Serial.println("Iniciando dados...");
  update_Data();
  Serial.println("Finalizando dados");
