        ...
        if(child_cnt == 0) {
            ...
            if(clip_corner == false) {
                ...
                if(lv_area_intersect(&bottom, &bottom, &clip_area_ori)) {
                    layer_children = lv_draw_layer_create(layer, LV_COLOR_FORMAT_NATIVE_ALPHA, &bottom);
                ...
                if(lv_area_intersect(&top, &top, &clip_area_ori)) {
                    layer_children = lv_draw_layer_create(layer, LV_COLOR_FORMAT_NATIVE_ALPHA, &top);
    ...
    if(layer_type == LV_LAYER_TYPE_NONE) {
        ...
        while(layer_area_act.y2 < layer_area_full.y2) {
            ...
            lv_layer_t * new_layer = lv_draw_layer_create(layer,
                                                          area_need_alpha ? LV_COLOR_FORMAT_NATIVE_ALPHA : LV_COLOR_FORMAT_NATIVE, &layer_area_act);
