...
void lv_draw_sw_layer(lv_draw_unit_t * draw_unit, const lv_draw_image_dsc_t * draw_dsc, const lv_area_t * coords)
{
    ...
    fill_dsc.color = lv_color_hex(layer_to_draw->color_format == LV_COLOR_FORMAT_NATIVE_ALPHA ? 0xff0000 : 0x00ff00);
                          ...
                          const lv_area_t * img_coords, const lv_area_t * clipped_img_area)
{
    ...
    /* check whether it is possible to accelerate the operation in synchronous mode */
        ...
        if((cf == LV_COLOR_FORMAT_L8) && (draw_dsc->recolor_opa > LV_OPA_MIN)) {
            cf_final = LV_COLOR_FORMAT_NATIVE_ALPHA;
        }
        if(transformed) {
            if(cf_final == LV_COLOR_FORMAT_RGB888 || cf_final == LV_COLOR_FORMAT_XRGB8888) cf_final = LV_COLOR_FORMAT_NATIVE_ALPHA;
