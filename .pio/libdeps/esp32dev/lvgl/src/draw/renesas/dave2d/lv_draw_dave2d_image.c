                          ...
                          const lv_area_t * img_coords, const lv_area_t * clipped_img_area)
{
    ...
    if(LV_COLOR_FORMAT_RGB565A8 == cf) {
        ...
        uint32_t size = header->h * (header->w * lv_color_format_get_size(LV_COLOR_FORMAT_NATIVE_ALPHA));
        ...
        d2_framebuffer(u->d2_handle,
                       ...
                       (d2_u32)header->w,
                       (d2_u32)header->h,
                       lv_draw_dave2d_lv_colour_fmt_to_d2_fmt(LV_COLOR_FORMAT_NATIVE_ALPHA));
