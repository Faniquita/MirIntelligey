...
void lv_lottie_set_buffer(lv_obj_t * obj, int32_t w, int32_t h, void * buf)
{
    ...
    int32_t stride = lv_draw_buf_width_to_stride(w, LV_COLOR_FORMAT_NATIVE_ALPHA);
    buf = lv_draw_buf_align(buf, LV_COLOR_FORMAT_NATIVE_ALPHA);
    ...
    lv_canvas_set_buffer(obj, buf, w, h, LV_COLOR_FORMAT_NATIVE_ALPHA);
...
{
    if(draw_buf->header.cf != LV_COLOR_FORMAT_NATIVE_ALPHA) {
